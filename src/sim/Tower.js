...
export function createTower(def, col, row, world, opts = {}) {
  return {
    ...
    maxLevel: 3,
    baseCost: def.cost || 0,
    upgradeCost: Math.ceil((def.cost || 0) * 0.75),
    repairCost: 0,
    sellRefund: Math.floor((def.cost || 0) * 0.5),
    ...
    contactRadius: def.contactRadius ?? def.blockRadius ?? (def.blocksEnemies ? 0.55 : 0),
    contactDamage: def.contactDamage ?? 0,
    contactTickRate: def.contactTickRate ?? 0,
    contactCd: 0,
    range: def.range,
    damage: def.damage,
    ...
    baseMaxHp: def.maxHp ?? def.hp ?? 0,
    baseDamage: def.damage,
    baseRange: def.range,
    baseContactDamage: def.contactDamage ?? 0,
    baseAttackRate: def.attackRate ?? def.fireRate,
    baseFireRate: def.fireRate,
    baseTriggerRadius: def.triggerRadius ?? null,
    baseCharges: def.charges ?? null,
    baseDuration: def.duration ?? null,
    color: def.color,
    alive: true,
