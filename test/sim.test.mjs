...
const NORTH_PATH_AFTER_CHOKE = { col: NORTH_CHOKE_CELL.col, row: NORTH_CHOKE_CELL.row + 1 };
const CORE_RESERVED_CELL = { col: LEVEL.core.col - 3, row: LEVEL.core.row - 3 };
const BLOCKED_TEST_CELL = { col: LEVEL.blockedZones[0].col, row: LEVEL.blockedZones[0].row };
...
section("warden barricade v1");
{
  ...
  w.marrow = 999;
  const placed = w.tryPlaceTower("barricade", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row);
...

{
  ...
  ok(dashWorld.tryPlaceTower("barricade", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row).ok, "Warden can build Barricade before using hero kit");
  ...
  ok(slamWorld.tryPlaceTower("barricade", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row).ok, "Warden can build Barricade before using Ward Slam");
  ...
  ok(attackWorld.tryPlaceTower("barricade", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row).ok, "Warden can build Barricade before using basic attack");
...
section("warden spike-gate v1");
{
  ...
  w.hero.respawnTimer = Infinity;
  w.marrow = 999;
  const placed = w.tryPlaceTower("spikegate", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row);
...

{
  const dashWorld = new World(LEVEL);
  dashWorld.marrow = 999;
  ok(dashWorld.tryPlaceTower("spikegate", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row).ok, "Warden can build Spike-gate before using hero kit");
  const x0 = dashWorld.hero.x;
  dashWorld.update(0.05, { moveX: 1, moveZ: 0, dash: true });
  run(dashWorld, 4, 0.05, { moveX: 1, moveZ: 0 });
  ...
  const slamWorld = new World(LEVEL);
  slamWorld.marrow = 999;
  ok(slamWorld.tryPlaceTower("spikegate", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row).ok, "Warden can build Spike-gate before using Ward Slam");
  const slamEnemy = spawnEnemyAt(slamWorld, "husk", slamWorld.defaultLaneId, NORTH_CHOKE_DIST);
  const slamCenterX = slamWorld.hero.x + Math.sin(slamWorld.hero.facing) * (slamWorld.hero.ability.centerOffset || 0);
  const slamCenterZ = slamWorld.hero.z + Math.cos(slamWorld.hero.facing) * (slamWorld.hero.ability.centerOffset || 0);
  slamEnemy.x = slamCenterX;
  ...
  const attackWorld = new World(LEVEL);
  attackWorld.marrow = 999;
  ok(attackWorld.tryPlaceTower("spikegate", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row).ok, "Warden can build Spike-gate before using basic attack");
  const attackEnemy = spawnEnemyAt(attackWorld, "husk", attackWorld.defaultLaneId, NORTH_CHOKE_DIST);
  attackEnemy.x = attackWorld.hero.x;
  attackEnemy.z = attackWorld.hero.z + 1.0;
  attackEnemy.hp = 100;
...
section("building rules");
{
  ...
  ok(!w.buildableAt(BLOCKED_TEST_CELL.col, BLOCKED_TEST_CELL.row), "cannot build on a blocked ruin/statue base");
  ok(!w.buildableAt(LEVEL.core.col, LEVEL.core.row), "cannot build on the core");
  ok(!w.buildableAt(CORE_RESERVED_CELL.col, CORE_RESERVED_CELL.row), "cannot build in the core reserved zone");
  ...
  const rBlocked = w.tryPlaceTower("ballista", BLOCKED_TEST_CELL.col, BLOCKED_TEST_CELL.row);
...

{
  const w = new World(LEVEL);
  w.marrow = 999;
  const placed = w.tryPlaceTower("ballista", NORTH_LANE.buildShoulders[0].col, NORTH_LANE.buildShoulders[0].row);
...

{
  const w = new World(LEVEL);
  w.marrow = 999;
  const placed = w.tryPlaceTower("trapstake", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row);
...

{
  const w = new World(LEVEL);
  w.marrow = 999;
  const placed = w.tryPlaceTower("censer", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row);
...

{
  ...
  poor.marrow = 999;
  const p = poor.tryPlaceTower("ballista", NORTH_LANE.buildShoulders[0].col, NORTH_LANE.buildShoulders[0].row);
...

{
  ...
  const trap = w.tryPlaceTower("trapstake", NORTH_PATH_AFTER_CHOKE.col, NORTH_PATH_AFTER_CHOKE.row).tower;
...

{
  ...
  w.hero.alive = false;
  w.hero.respawnTimer = Infinity;
  const placed = w.tryPlaceTower("barricade", NORTH_LANE.buildShoulders[0].col, NORTH_LANE.buildShoulders[0].row);
...

{
  for (const towerId of ["trapstake", "censer", "ballista"]) {
    ...
    w.hero.alive = false;
    w.hero.respawnTimer = Infinity;
    const placed = w.tryPlaceTower(towerId, NORTH_LANE.buildShoulders[0].col, NORTH_LANE.buildShoulders[0].row);
...

{
  ...
  first.laneOffset = 0;
  second.laneOffset = 0;
  const firstPos = pointAtDistance(w.lane, first.dist);
  const secondPos = pointAtDistance(w.lane, second.dist);
  first.x = firstPos.x;
  first.z = firstPos.z;
  second.x = secondPos.x;
  second.z = secondPos.z;
...

{
  ...
  w.hero.alive = false;
  w.hero.respawnTimer = Infinity;
  const placed = w.tryPlaceTower("brazier", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row);
...
section("a tower kills enemies and grants marrow");
{
  ...
  w.hero.alive = false;
  w.hero.respawnTimer = Infinity;
  const place = w.tryPlaceTower("ballista", NORTH_LANE.buildShoulders[0].col, NORTH_LANE.buildShoulders[0].row);
...
section("manual hero attack and slam damages");
{
  ...
  const tower = w.tryPlaceTower("barricade", NORTH_CHOKE_CELL.col, NORTH_CHOKE_CELL.row).tower;
...
section("object pooling reuses dead enemies");
{
  ...
  w.hero.alive = false;
  w.hero.respawnTimer = Infinity;
  w.tryPlaceTower("ballista", NORTH_LANE.buildShoulders[0].col, NORTH_LANE.buildShoulders[0].row);
  w.tryPlaceTower("spire", NORTH_LANE.buildShoulders[1].col, NORTH_LANE.buildShoulders[1].row);
  w.startWave();
  run(w, 4000, 0.05, {});
